...
The same [not measured] sample databases — [not measured] rows across [not measured] tables — loaded into both engines from the same `mysqldump` files.
