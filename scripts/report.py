#!/usr/bin/env python3
"""The MySQL/Dolt report: every database, both engines, the ratio, from build/results.json.

  python3 scripts/report.py        prints it

It writes no file. scripts/render.py owns every document and embeds this report in REPORT.md as a
block (`{{block:dolt_report}}`); two writers for one file is how a generated document acquires a
hand-edited section nobody notices.
"""
import argparse, json, os, sys

sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from common import ROOT, VERSIONS, human, load_results, duration  # noqa: E402

RAW = {}
GENERATED = ("<!-- Generated by scripts/report.py from build/results.json and embedded in REPORT.md\n"
             "     by scripts/render.py. Do not edit by hand: `make docs` regenerates it. -->")
# Carried in all three documents. Someone deciding how much weight to give these numbers should
# know how they were produced before reading any of them, and REPORT.md is the one most likely to
# be read on its own, without the README's framing around it.
HUMAN_NOTE = (
...
def environment_table():
    ...
    return "\n".join([
        ...
        f"| DoltLite | `{g.get('doltlite_image', '')}` — {g.get('doltlite_version', 'not recorded')}, named by package checksums, beside sqlite3 {g.get('sqlite3_version', 'not recorded')} |",
        f"| Tuning | {g['tuning']} |",
...
def report(items):
    ...
    L = [GENERATED, "", f"# Disk usage: MySQL {VERSIONS['mysql']['version']} against Dolt {VERSIONS['dolt']['version']}", "",
         HUMAN_NOTE, "",
    ...
    with_stats = sum(1 for r in RAW.values() if r.get("server_stats_bytes"))
    ...
    L += ["## What a running server adds, and why it is not in the figures", "",
          ...
          (f"Measured on a copy: starting a server and reading every table in every database wrote "
           f"**{human(served)}** in total, {human(served / with_stats)} per database."
           if served and with_stats else
          ...
          "deployment will use, and because the floor would be a misleading thing to remember.", ""]
...
def main():
    """Print the MySQL/Dolt report. REPORT.md itself is rendered by scripts/render.py from
    docs/templates/REPORT.md, which embeds this report as a block; two writers for one file is how a
    generated document acquires a hand-edited section nobody notices."""
