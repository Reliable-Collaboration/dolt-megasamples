#!/usr/bin/env python3
"""Turn the measurements into REPORT.md.

  python3 scripts/report.py [--check]

Writes `REPORT.md` -- every database, both engines, the ratio -- is generated from
`build/results.json`, so its numbers are the numbers that were measured. It no longer writes any
part of README.md: scripts/render.py owns every document, and two writers for one file is how a
...
def environment_table():
    ...
    return "\n".join([
        ...
        f"| MySQL | `{g['mysql_image']}` — {g['mysql_version']}, pinned by image tag |",
        f"| Dolt | `{g['dolt_image'].split('@')[0]}` — {g['dolt_version']}, pinned by image digest |",
        f"| PostgreSQL | `{g.get('postgres_image', '').split('@')[0]}` — {g.get('postgres_version', 'not recorded')}, pinned by image digest |",
        f"| DoltgreSQL | `{g.get('doltgres_image', '').split('@')[0]}` — {g.get('doltgres_version', 'not recorded')}, pinned by image digest |",
        f"| DoltLite | `{g.get('doltlite_image', '')}` — {g.get('doltlite_version', 'not recorded')}, pinned by package checksums, beside sqlite3 {g.get('sqlite3_version', 'not recorded')} |",
        f"| Tuning | {g['tuning']} |",
...
def main():
    ...
    text = report(items)
    # README.md is not written here any more. scripts/render.py owns every document, generating it
    # from docs/templates and the measurement files, and two writers for one file is how a
    # generated document acquires a hand-edited section nobody notices.
    if a.check:
        stale = []
