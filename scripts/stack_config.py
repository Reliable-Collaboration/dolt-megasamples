...
def main():
    ...
    e = serve["engines"]
    # the engines' images come from versions.json (one version per result set), so the stack serves
    # the version the stores were written and measured with; compose.yaml's image lines are defaults
    from common import DOLT_IMAGE
    from pairs import DOLTGRES_IMAGE, LITE_IMAGE
    lines = ["# Generated by scripts/stack_config.py on every `make up` -- do not edit.",
             ...
             "# The engine images are the versions in versions.json, the ones every number was measured on.",
             "services:",
             "  dolt:", f"    image: {DOLT_IMAGE}", f"    mem_limit: {mem['dolt']}", "    volumes:",
             "      - ./data/dolt-serve:/var/lib/dolt"]
    lines += [f"      - {p}:/var/lib/dolt/{db}" for db, p in sorted(e["dolt"]["paths"].items())]
    lines += ["  doltgres:", f"    image: {DOLTGRES_IMAGE}", f"    mem_limit: {mem['doltgres']}", "    volumes:",
              "      - ./data/doltgres-serve:/var/lib/doltgres"]
    lines += [f"      - {p}:/var/lib/doltgres/{db}" for db, p in sorted(e["doltgres"]["paths"].items())]
    lines += ["  doltgres-init:", f"    image: {DOLTGRES_IMAGE}"]
    lines += ["  doltlite:", f"    image: {LITE_IMAGE}", "    volumes:", "      - ./data/doltlite-serve:/data"]
