...
Measured on the {{corpus.databases}} sample databases of
[`sql-megasamples`](https://github.com/Reliable-Collaboration/sql-megasamples), {{corpus.rows}} rows
across {{corpus.tables}} tables, each pair loaded from its own engine's dump. Every number here is
generated from the measurements; none was typed by a person.
...
* [What was measured](#what-was-measured) -- the versions and the five tests
* [What would make a reviewer hesitate](#what-would-make-a-reviewer-hesitate)
* [The machine](#the-machine), [reproducing this](#reproducing-this), [running the databases](#running-the-databases), [layout](#layout), [licence](#licence)
...
Each of the three Dolt engines is a SQL database with Git-like versioning that speaks the wire
protocol of the engine it stands in for, so the same clients work against both. Each stores data in
an entirely different way from its counterpart: InnoDB, PostgreSQL's heap and SQLite's B-tree write
pages, while the Dolt engines write content-addressed chunks into prolly trees and keep the history
of every change. Those are different enough that "how big will it be, and how long will it take?" is
not answerable by reasoning about it, so this measures it, on the {{corpus.databases}} sample
databases of `sql-megasamples` -- {{corpus.rows}} rows of real, varied, publicly-licensed data
across {{corpus.tables}} tables, from teaching schemas of a few hundred rows to a star schema of
several million -- from that corpus's own MySQL, PostgreSQL and SQLite ports.
